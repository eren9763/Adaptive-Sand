...
flags.DEFINE_enum("algo", "sand", ["sa", "lly", "seql", "gl", "sand"], "Algorithm")
flags.DEFINE_integer(
...
flags.DEFINE_float("sigma", 1.5, "SAND sigma")
...
def run_trial(
  ...
  if FLAGS.algo in ALGOS:
    ...
    elif FLAGS.algo == "sand":
      selected_indices = mlp_select.selected_indices().numpy().tolist()
    assert (
  ...
  if FLAGS.algo == "sand": # skip retraining
    mlp_fit = mlp_select
  ...
  print(FLAGS.algo, results["val"], results["test"])
